...
class StorageIO {
  ...
  Future<File> downloadImg(String imgName) async {
    ...
    final StorageReference storageReference = FirebaseStorage.instance.ref().child('Images/$imgName');
  ...
  Future<String> uploadImage({@required File imageFile, @required String userId, String folder : 'ProfilePics'}) async {
    final RegExp regExp = RegExp('\\.[0-9a-z]+\$');
    final String format = regExp.stringMatch(imageFile.path);

    final fileName = userId + format;

    final StorageReference ref = FirebaseStorage.instance.ref().child('Images/$folder/$fileName');
